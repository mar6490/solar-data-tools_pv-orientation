...
def find_fit_costheta(data_matrix, clear_index, doy):
    ...
    if len(data) > 2:
        ...
        phi3 = 1e-6 * cvx.norm1(x3)
    solstice_index = solstice_centered_index(doy)
    for val in set(solstice_index):
        day_mask = solstice_index == val
        if np.sum(day_mask) > 2:
            constraints.append(cvx.diff(x3[day_mask]) == 0)
    constraints.append(x3 <= 0)
    if len(data) > 2:
        phi4 = 1e3 * cvx.sum_squares(cvx.diff(x4, k=2)) + 1e-3 * cvx.sum_squares(x4)
    else:
        phi4 = 1e-3 * cvx.sum_squares(x4)
